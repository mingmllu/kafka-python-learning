...
topic = 'test-delay'
consumer = KafkaConsumer(topic, bootstrap_servers=brokers, auto_commit_interval_ms=100)
...
for message in consumer:
    ...
    dt_from_kafka_to_consumer = time.time() - data_dict["timestamp"]
    ...
    print("---------------- message.offset %d message offset gap: %d" % (message.offset, message.offset - message_offset))
    message_offset = message.offset
    ...
    if max_dt_from_kafka_to_consumer > 1:
        break
